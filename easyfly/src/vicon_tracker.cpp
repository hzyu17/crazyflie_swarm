	...
	//,swarm_vel_filters(g_vehicle_num)
    {
        ...
        for(int i=0;i<g_vehicle_num;i++){
			...
			/*initialize frame*/
				...
				vel_boundary(0) = 0.6f;
				vel_boundary(1) = 0.6f;
				vel_boundary(2) = 0.35f;
			/*Velocity_filter* velFilter = new Velocity_filter(&swarm_pos_shift_observ[i],&swarm_pos_shift[i],&swarm_pos_shift_buffer[i]);
			swarm_vel_filters.push_back(velFilter);*/
			//Velocity_filter velFilter(&swarm_pos_shift_observ[i],&swarm_pos_shift[i],&swarm_pos_shift_buffer[i]);
			//initrd.imgswarm_vel_filters.push_back(velFilter);
...
private:
    ...
    std::vector<ros::Publisher> m_vicon_markernumpub,m_pos_est_v;
    ...
    std::vector<ros::Subscriber> m_imusub_v;
    ...
    easyfly::pos_est_test m_pos_estmsg;
	bool isFirstVicon;
...
void ViconTracker::unite(vector<float> &x_init_pos,vector<float> &y_init_pos,vector<float> &x_marker_pos,vector<float> &y_marker_pos)
	{
		...
		for(int i=0;i<x_marker_pos.size();++i)//choose the first point
		{
			...
			if(min_dstc[1]>min_dstc[0]){
                ...
                max_dstc=min_dstc[1];
				/*temp_exch = num_of_point[2];

				num_of_point[2] = num_of_point[0];
				printf("------- num_of_point[2]: %d \n ------- num_of_point[0]: %d ------\n",num_of_point[2],num_of_point[0]);*/
				//num_of_point[0] = temp_exch;
		    }
...
//void ViconTracker::onMouse(int event, int x, int y, int, void* userInput)
...
void ViconTracker::vicon_markerCallback(const vicon_bridge::Markers::ConstPtr& msg)
	{	
		...
		{	
			...
			//re-order the position sequence
			...
			for(int i=0;i<g_vehicle_num;++i){
				for(int pt_relative=0;pt_relative<4;++pt_relative){
					printf("****************intial relative  %d  %f   %f   %f ********************\n",i
																					  ,m_swarmFrame[i].pts_relative[pt_relative](0)
																				 	  ,m_swarmFrame[i].pts_relative[pt_relative](1)
																				 	  ,m_swarmFrame[i].pts_relative[pt_relative](2));
				}
		...
		else if(!isFirstVicon && msg->markers.size() != 0)
		{
	    	...
	    	for (int i = 0; i < g_vehicle_num; ++i)//for every vehicles
	    	{
				...
				swarm_pos_predict[i] = swarm_pos[i] + swarm_pos_shift[i] + REVISE_WEIGHT*swarm_pos_err[i]/(REVISE_WEIGHT+swarm_err_abs[i]);
	    	...
	    	for (int i = 0; i < swarm_pos.size(); ++i)
	    	{
				...
				m_pos_estmsg.vel_est.x = swarm_pos_shift[i](0)/vicon_dt;
				m_pos_estmsg.vel_est.y = swarm_pos_shift[i](1)/vicon_dt;
				m_pos_estmsg.vel_est.z = swarm_pos_shift[i](2)/vicon_dt;
...
Vector3f ViconTracker::onePt_predict(Vector3f& pt,const int i)
{
	...
	{	
		...
		if(vec3f_dot(&new_relative,&m_swarmFrame[i].pts_relative[pt_relative])>dot){

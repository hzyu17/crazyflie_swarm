...
private:
	...
	ros::Subscriber m_estsub, m_magneticsub, m_accsub;
...
public:
	...
	//	m_group_index = node_index;
		char msg_name[50];
		sprintf(msg_name,"/num_vehiclepub",m_group_index);
  		num_vehiclepub = nh.subscribe<crazyflie_driver::num_vehiclepub>(msg_name,5,&Swarm_Controller::num_veh_Callback, this);
		...
		m_estsub = nh.subscribe<easyfly::state_est>(msg_name,5,&Swarm_Controller::estCallback, this);
	...
	void estCallback(const easyfly::state_est::ConstPtr& est)
	{
		...
		m_est.m_pitch_est = est->pitch_est;
		m_est.m_roll_est = est->roll_est
	}
...
//  int ret = init_scan(argc, argv);
	...
	int index = indexc;
	char node_name[50];
	sprintf(node_name, "Swarm_Controller%d", index);
